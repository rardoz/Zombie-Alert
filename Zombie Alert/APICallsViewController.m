...
- (void)apiDialogFeedUser {
  ...
  //  url = [prefs stringForKey:@"zombieurl"];
    ...
    NSArray* actionLinks = [NSArray arrayWithObjects:[NSDictionary dictionaryWithObjectsAndKeys:
                                                           @"Zombie Alert!",@"name",[prefs objectForKey:@"zombieurl"],@"link", nil], nil];
    NSString *actionLinksStr = [jsonWriter stringWithObject:actionLinks];
    // Dialog parameters
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   @"I'm using Zombie Alert for iOS app", @"name",
                                   @"Zombie Alert for iPhone", @"caption",
                                   @"Check out Zombie Alert for iOS to keep track of zombie attacks in your area!", @"description",
                                   [prefs objectForKey:@"zombieurl"], @"link",
                                  [prefs objectForKey:@"zombiestoryimg"], @"picture",
...
- (void)request:(FBRequest *)request didLoad:(id)result {
    ...
    switch (currentAPICall) {
        case kAPIGraphUserPermissionsDelete:
        {
            ...
            Settings *rootViewController = (Settings *)[[self.navigationController viewControllers] objectAtIndex:0];
    ...
    switch (currentAPICall) {
        ...
        case kDialogFeedFriend:
        {
            ...
            if ([params valueForKey:@"post_id"]) {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Success" message:@"Published feed successfully!" delegate:self cancelButtonTitle:@"Your Welcome" otherButtonTitles: nil];
                [alert show];
                [alert release];
            }
            break;
...
- (void)dialog:(FBDialog*)dialog didFailWithError:(NSError *)error {
    NSLog(@"Error message: %@", [[error userInfo] objectForKey:@"error_msg"]);
    [self showMessage:@"Oops, something went haywire."];
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"FAIL" message:@"Published feed was not done successfully!" delegate:self cancelButtonTitle:@"Sorry" otherButtonTitles: nil];
    [alert show];
    [alert release];
...
- (void)userDidGrantPermission {
    NSLog(@"running user did grant permisssion");
