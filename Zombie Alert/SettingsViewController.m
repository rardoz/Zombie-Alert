...
- (void)backBtnUserClicked:(id)object {
    ...
    NSLog(@"close this view save and set the bool for the tumblr switch and store it.");
     
}

-(void)viewWillAppear:(BOOL)animated {
	self.navigationItem.title=@"Sign Into Tumblr!";
	UIBarButtonItem *settingsButton = [[UIBarButtonItem alloc] initWithTitle:@"Cancel" style:UIBarButtonItemStyleBordered target:self action:@selector(backBtnUserClicked:)];
...
-(void)saveUserData {
    ...
    [prefs setInteger:1 forKey:@"tumblron"];
	[prefs synchronize];
    NSLog(@"saved user data for tumblr!");
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
	if (alertView == successView) {
        ...
        [self dismissModalViewControllerAnimated:YES];        //hey you righ there
